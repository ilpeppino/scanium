...
This document explains how to use the diagnostics tools to debug and validate the Camera UI FTUE (First-Time User Experience) feature.
...
**Solution**: This PR adds `ftuePulse` modifier to all buttons. If still not visible, increase scale (1.08 → 1.12) or duration.
...
**Solution**: Ensure `ftueAnchor` is applied to the actual button surface, not a parent container that might be size 0.
...
- `/androidApp/src/main/java/com/scanium/app/camera/CameraControlsOverlay.kt` (where anchors are attached)
...
1. **If anchors are NULL**: Fix anchor registration (ensure buttons are composed with correct modifiers)
