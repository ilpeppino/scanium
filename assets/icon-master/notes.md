...
- **scanium_mark_master.svg** — Full-color vector mark with gradients, chrome effects, and lens
  highlights. Source for all icon variants.
- **scanium_mark_monochrome.svg** — Simplified single-color (black) vector for Android 13+ themed
...
2. **Curved S**: Blue flowing S-shape connecting the two lenses, representing the "Scanium" brand
   identity.
...
- [ ] `ic_launcher_monochrome.xml` renders as single-color (no gradients visible in Android 13+
  theme engine).
...
- [ ] AndroidManifest.xml correctly references `@mipmap/ic_launcher` and
