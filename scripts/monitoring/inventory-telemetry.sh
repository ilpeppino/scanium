...
***REMOVED*** =============================================================================
***REMOVED*** inventory-telemetry.sh - Discover telemetry in running LGTM stack
***REMOVED*** =============================================================================
***REMOVED*** Queries Grafana's datasource proxy to discover:
***REMOVED***   - Available datasources (Loki, Tempo, Mimir)
***REMOVED***   - Metric names and labels from Mimir
***REMOVED***   - Log labels and values from Loki
***REMOVED***   - Trace services/operations from Tempo (if available)
***REMOVED***
***REMOVED*** Usage:
***REMOVED***   ./inventory-telemetry.sh [options]
***REMOVED***
***REMOVED*** Options:
***REMOVED***   -u, --grafana-url URL   Grafana URL (default: http://localhost:3000)
***REMOVED***   -t, --token TOKEN       Grafana API token (optional, anonymous auth used if not set)
***REMOVED***   -o, --output-dir DIR    Output directory (default: monitoring/grafana)
***REMOVED***   -h, --help              Show this help message
***REMOVED***
***REMOVED*** Outputs:
***REMOVED***   - telemetry-inventory.json (machine-readable)
***REMOVED***   - telemetry-inventory.md (human-readable summary)
***REMOVED***
***REMOVED*** Safety:
***REMOVED***   - Uses small time ranges (last 15m) to avoid heavy queries
***REMOVED***   - Limits label value queries to prevent cardinality explosions
***REMOVED***   - Timeouts prevent hung requests
***REMOVED*** =============================================================================
...
show_help() {
    head -40 "$0" | tail -35 | sed 's/^***REMOVED*** //' | sed 's/^***REMOVED***//'
    exit 0
}

***REMOVED*** Make a Grafana API request
grafana_api() {
    local path="$1"
    local auth_header=""
...
***REMOVED*** Query Mimir via Grafana datasource proxy
query_mimir() {
    local query="$1"
    local endpoint="${2:-query}"

    ***REMOVED*** URL encode the query
    local encoded_query
...
***REMOVED*** Query Loki via Grafana datasource proxy
query_loki() {
    local path="$1"
    grafana_api "/api/datasources/proxy/uid/LOKI/loki/api/v1/${path}"
}

***REMOVED*** Query Tempo via Grafana datasource proxy
query_tempo() {
    local path="$1"
...
***REMOVED*** Set default output dir
...
***REMOVED*** Check Grafana connectivity
...
***REMOVED*** Extract specific datasources
...
***REMOVED*** =============================================================================
***REMOVED*** Discover Mimir Metrics
***REMOVED*** =============================================================================
...
if [[ "$has_mimir" == "true" ]]; then
    ...
    ***REMOVED*** Get metric names (limited sample)
    ...
    if echo "$metric_names_raw" | jq -e '.status == "success"' &>/dev/null; then
        ...
        ***REMOVED*** Discover labels from key metrics
        ...
        if [[ -n "$http_metric" ]]; then
            ...
            if echo "$http_labels_raw" | jq -e '.status == "success"' &>/dev/null; then
                ...
                ***REMOVED*** Sample label values for key labels
                label_values='{}'
        ...
        ***REMOVED*** Check for pipeline metrics labels
        if [[ ${***REMOVED***otel_metrics[@]} -gt 0 ]]; then
...
if [[ "$has_loki" == "true" ]]; then
    ...
    ***REMOVED*** Get label names
    ...
    if echo "$labels_raw" | jq -e '.status == "success"' &>/dev/null; then
        ...
        ***REMOVED*** Sample recent log streams
...
if [[ "$has_tempo" == "true" ]]; then
    ...
    ***REMOVED*** Try to get services via Tempo API
    ***REMOVED*** Note: Tempo's search API may vary by version
    ...
    if echo "$services_raw" | jq -e '.tagNames' &>/dev/null; then
        ...
        ***REMOVED*** Try to get service names
        services='[]'
        ...
        ***REMOVED*** Try to get span names/operations
        operations='[]'
        ...
        ***REMOVED*** Fallback: check if Tempo responds at all
...
***REMOVED*** =============================================================================
***REMOVED*** Discover Existing Dashboards
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** Build Inventory Report
***REMOVED*** =============================================================================
...
***REMOVED*** =============================================================================
***REMOVED*** Write Output Files
***REMOVED*** =============================================================================

mkdir -p "$OUTPUT_DIR"

***REMOVED*** Write JSON inventory
...
***REMOVED*** Write Markdown summary
...
SECTION
    ...
    if [[ "$http_metric" != "none" && "$http_metric" != "" ]]; then
        echo "***REMOVED******REMOVED******REMOVED*** HTTP Metrics" >> "$md_file"
        echo "" >> "$md_file"
    ...
    ***REMOVED*** List key metric categories
    echo "***REMOVED******REMOVED******REMOVED*** Metric Categories" >> "$md_file"
    echo "" >> "$md_file"
    echo "| Category | Sample Metrics |" >> "$md_file"
    echo "|----------|----------------|" >> "$md_file"

    ***REMOVED*** HTTP metrics
    ...
    ***REMOVED*** Process metrics
    ...
    echo "***REMOVED******REMOVED*** Metrics (Mimir)" >> "$md_file"
    echo "" >> "$md_file"
...
SECTION
    ...
    echo "***REMOVED******REMOVED*** Logs (Loki)" >> "$md_file"
    echo "" >> "$md_file"
    echo "> Loki not available or no logs found." >> "$md_file"
    echo "" >> "$md_file"
fi

***REMOVED*** Traces section
...
SECTION
    ...
    echo "***REMOVED******REMOVED*** Traces (Tempo)" >> "$md_file"
    echo "" >> "$md_file"
    echo "> Tempo not available or no traces found." >> "$md_file"
    echo "" >> "$md_file"
fi

***REMOVED*** Dashboards section
cat >> "$md_file" << 'SECTION'
...
cat >> "$md_file" << 'SECTION'
***REMOVED******REMOVED*** Dashboard Recommendations

Based on the inventory, here are the recommended dashboards:

***REMOVED******REMOVED******REMOVED*** For Backend Monitoring (if HTTP metrics exist)
...
***REMOVED******REMOVED******REMOVED*** For Log Analysis
- **Logs Explorer** - Volume, error rates, pattern analysis

***REMOVED******REMOVED******REMOVED*** For Trace Analysis
