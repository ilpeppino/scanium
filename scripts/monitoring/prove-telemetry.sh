#!/bin/bash
# prove-telemetry.sh
# Proves whether telemetry data exists in Grafana datasources
# Non-negotiable: Does NOT leak secrets
...
# Read Grafana token securely (never echo)
read_token() {
...
query_loki() {
    ...
    # Query for label keys (labels API)
    curl -s -X GET \
         -H "Authorization: Bearer $token" \
...
query_tempo() {
    ...
    # Query for services (Tempo search API)
    curl -s -X GET \
         -H "Authorization: Bearer $token" \
...
main() {
    ...
    # ========================================================================
    # Test Mimir (Prometheus)
    # ========================================================================
    if [[ -n "$MIMIR_UID" && "$MIMIR_UID" != "null" ]]; then
        log "Testing Mimir..."

        # Query for 'up' metric (last 15 minutes)
        MIMIR_QUERY='{
            "queries": [{
                "refId": "A",
                "expr": "up",
