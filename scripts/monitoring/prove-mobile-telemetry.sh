***REMOVED***!/bin/bash
***REMOVED***
***REMOVED*** prove-mobile-telemetry.sh
***REMOVED*** Verifies that mobile telemetry (OTLP logs) is reaching Loki
***REMOVED***
***REMOVED*** Usage: ./scripts/monitoring/prove-mobile-telemetry.sh
...
***REMOVED*** Calculate time range (last 15 minutes)
END_TIME=$(date +%s)
...
***REMOVED*** Query Loki via SSH to NAS
...
***REMOVED*** Check if query succeeded
...
***REMOVED*** Count total log entries
...
if [ "$TOTAL_LOGS" -eq 0 ]; then
  ...
  echo "  - Verify OTLP endpoint: curl http://REDACTED_INTERNAL_IP:4318/health"
...
***REMOVED*** Extract event_name values and count occurrences
echo "$RESULT" | jq -r '
...
***REMOVED*** Extract platform values and count occurrences
echo "$RESULT" | jq -r '
...
***REMOVED*** Extract app_version values (unique)
echo "$RESULT" | jq -r '
...
***REMOVED*** Extract build_type values and count occurrences
echo "$RESULT" | jq -r '
...
echo ""
echo "Next steps:"
echo "  - Open Grafana: http://REDACTED_INTERNAL_IP:3000"
