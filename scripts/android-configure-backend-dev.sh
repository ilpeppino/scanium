#!/bin/bash
##############################################################################
# Scanium Android Backend Configuration Script
#
# Purpose: Configure the Android app to connect to a backend server.
#          Writes configuration to local.properties (never committed to git).
#
# Usage:
#   ./scripts/android-configure-backend-dev.sh [OPTIONS]
#
# Options:
...
#
# Examples:
#   # Configure via Cloudflare hostname
#   ./scripts/android-configure-backend-dev.sh \
#       --url https://scanium-api.example.com \
#       --key your-api-key
#
#   # Configure with separate LAN URL for debug builds
#   ./scripts/android-configure-backend-dev.sh \
#       --url https://scanium-api.example.com \
...
#   export SCANIUM_API_BASE_URL=https://scanium-api.example.com
...
# Fall back to environment variables if not provided as arguments
...
if [ ! -f "$LOCAL_PROPS" ]; then
    echo "# Scanium local configuration" > "$LOCAL_PROPS"
    echo "# This file is NOT committed to git" >> "$LOCAL_PROPS"
    echo "" >> "$LOCAL_PROPS"
fi
...
update_property() {
    ...
    if grep -q "^${key}=" "$file" 2>/dev/null; then
        # Update existing property (macOS-compatible sed)
