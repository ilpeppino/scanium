...
# Deterministic build + install + verify script for prodDebug variant
# Ensures the installed APK always matches the current git HEAD SHA
#
# Usage:
#   ./scripts/dev/install_prod_debug.sh [--uninstall]
#
# Options:
...
# Parse the build log
# Format: versionName=X.X.X versionCode=XXX flavor=prod buildType=debug git=XXXXXXX time=YYYY-MM-DDTHH:MM:SSZ
