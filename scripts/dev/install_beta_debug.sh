...
INSTALLED_VERSION_NAME=$(echo "$BUILD_LOG" | grep -oP 'versionName=\K[^ ]+' || echo "unknown")
INSTALLED_VERSION_CODE=$(echo "$BUILD_LOG" | grep -oP 'versionCode=\K[^ ]+' || echo "unknown")
INSTALLED_FLAVOR=$(echo "$BUILD_LOG" | grep -oP 'flavor=\K[^ ]+' || echo "unknown")
INSTALLED_BUILD_TYPE=$(echo "$BUILD_LOG" | grep -oP 'buildType=\K[^ ]+' || echo "unknown")
INSTALLED_SHA=$(echo "$BUILD_LOG" | grep -oP 'git=\K[^ ]+' || echo "unknown")
INSTALLED_BUILD_TIME=$(echo "$BUILD_LOG" | grep -oP 'time=\K[^ ]+' || echo "unknown")
