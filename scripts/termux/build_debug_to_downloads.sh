...
***REMOVED*** Build Scanium debug APK and copy to Android Downloads for easy installation.
***REMOVED*** Designed for Termux on Android devices.
...
***REMOVED*** Determine which Gradle task to run
GRADLE_TASK=":androidApp:assembleDebug"

***REMOVED*** Check if androidApp module exists, otherwise use generic assembleDebug
...
***REMOVED*** Find the newest debug APK in common output locations
APK_PATH=""
SEARCH_DIRS=(
...
for dir in "${SEARCH_DIRS[@]}"; do
    if [[ -d "$dir" ]]; then
        ***REMOVED*** Find newest *-debug.apk or *debug*.apk
...
***REMOVED*** Get APK filename
APK_FILENAME=$(basename "$APK_PATH")

***REMOVED*** Copy to Downloads
cp -f "$APK_PATH" "$DEST_DIR/"

***REMOVED*** Get file size
