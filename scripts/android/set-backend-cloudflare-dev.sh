#!/bin/bash
##############################################################################
# Set Backend to Cloudflare (Development)
#
# Configures local.properties to use the Cloudflare-exposed NAS backend
# at https://scanium.gtemp1.com for devDebug builds.
#
# Usage:
#   export SCANIUM_API_KEY="your-api-key"
#   ./scripts/android/set-backend-cloudflare-dev.sh
#
# Or provide URL override:
#   SCANIUM_API_BASE_URL_DEBUG=https://custom.example.com \
#   SCANIUM_API_KEY="your-key" \
#   ./scripts/android/set-backend-cloudflare-dev.sh
#
##############################################################################
...
# Default Cloudflare backend URL
DEFAULT_BACKEND_URL="https://scanium.gtemp1.com"

# Get values from env or defaults
...
update_property() {
    ...
    if grep -q "^${key}=" "$file" 2>/dev/null; then
        # Update existing (macOS sed compatible)
...
# Set debug URL (primary for devDebug builds)
update_property "scanium.api.base.url.debug" "$BACKEND_URL" "$LOCAL_PROPS"

# Also set main URL as fallback
