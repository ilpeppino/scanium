...
class PriceEstimationOrchestratorTest {
    ...
    @Test
    fun `dedup prevents duplicate provider calls for same request`() =
        runTest {
    ...
    @Test
    fun `status transitions reach Ready`() =
        runTest {
            val provider = CountingProvider()
    ...
    @Test
    fun `mock provider returns deterministic values per category`() =
        runTest {
    ...
    @Test
    fun `timeout yields failed status`() =
        runTest {
