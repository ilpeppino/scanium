...
The KMP (Kotlin Multiplatform) migration has moved core business logic from Android-only modules (`com.scanium.app.*`) to shared KMP modules (`com.scanium.core.*`). This has created a critical disconnect between the test suite and the refactored codebase:
...
- **Test failures**: Tests importing from legacy packages that have been superseded by KMP implementations

**Goal**: Systematically migrate all unit tests to align with the KMP architecture, ensure 100% test coverage for shared business logic, and eliminate the legacy test duplication.
...
**Problem**: 90% of androidApp tests still use legacy imports and will fail to compile or run against the migrated codebase.
...
- Complex logic (ObjectTracker, ItemAggregator) has ~30 tests in androidApp but only 1-2 in commonTest
