...
**Important:** Run these scripts from inside Termux on Android. The shebang (`#!/usr/bin/env bash`) is portable across macOS and Termux, but storage paths like `$HOME/storage` only exist in Termux.
