# Pull Request: Fix ML Kit Model Download Loading State (Issue #018)
...
# Optionally throttle network to simulate slow download
# Settings > Developer Options > Network throttling > Slow 3G

# Launch app
# Expected: Loading overlay with explanation
...
# Tap "Retry"
# Expected: Loading overlay, then camera works
```

**3. Already Downloaded Test:**
```bash
# Launch app (models already present)
# Expected: Brief loading (<1 second), immediate camera access
```
...
- Related: Issue #016 (Camera error handling)
