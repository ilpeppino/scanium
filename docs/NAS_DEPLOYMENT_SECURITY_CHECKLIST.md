# NAS Deployment Security Checklist
...
- [ ] **Generate strong passwords** (minimum 32 characters)
  ...
  # Session signing secret (64 bytes)
...
- [ ] **Verify env files are gitignored**
  ...
  # Should output the path (ignored)
  ```
...
- [ ] **Verify port bindings in docker-compose**
  ...
  # WRONG: Publicly accessible
  ports:
    - "3100:3100"  # This binds to 0.0.0.0
  ```
...
- [ ] **No privileged containers**
  ```bash
  # Check for privileged mode (should return nothing)
  grep -r "privileged: true" deploy/nas/compose/
  ```

- [ ] **No host networking**
  ```bash
  # Check for host network (should return nothing)
...
- [ ] **Read-only config mounts**
  ```yaml
  volumes:
    - /path/to/config.yaml:/etc/app/config.yaml:ro  # :ro = read-only
  ```
...
- [ ] **Viewers cannot edit dashboards**
  ```env
  GF_AUTH_ANONYMOUS_ORG_ROLE=Viewer  # Not Admin
  ```

---

### 6. Cloudflare Tunnel Security
...
- [ ] **Docker images pinned to specific versions**
  ```yaml
  image: grafana/grafana:10.3.1  # GOOD: pinned
  image: grafana/grafana:latest   # BAD: unpredictable
  ```
...
```bash
# 1. Check no secrets in environment
docker exec scanium-api env | grep -i password
# Should show masked values only

# 2. Check port bindings
docker port scanium-loki
# Should be empty (internal only)

docker port scanium-grafana
# Should show 0.0.0.0:3000 (intentionally exposed)

# 3. Check Grafana auth
...
- [ ] **nmap scan from external network**
  ...
  # Only Cloudflare Tunnel port should be open
  ```
...
## Addendum: Issues Identified in 2026-01-05 Review

The following issues were identified and should be addressed:

| Issue | Priority | Description |
|-------|----------|-------------|
| #361 | P1 | NAS monitoring containers use `restart: "no"` - change to `unless-stopped` |
| #362 | P2 | Alloy admin UI port 12345 exposed - bind to localhost |
| #363 | P2 | Tempo max_workers=100 too high - reduce to 4 |
| #364 | P2 | Prometheus scrape interval 15s - increase to 60s |
| #367 | P2 | NAS Grafana auth not explicitly enforced |
| #368 | P3 | Create NAS-specific config overlays |
