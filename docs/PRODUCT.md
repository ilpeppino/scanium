...
- Mock selling flow: select items and list via fake marketplace service; no real network calls for selling (UI temporarily disabled).
...
- **Selling**: `SellScreen` lets users pick items and run the mocked listing flow (currently hidden in UI).
