...
2. **Recommended monitoring:**
   - Add cron job to run smoke test every 5 minutes
   - Example: `*/5 * * * * /path/to/smoke.sh >> /var/log/scanium-smoke.log 2>&1`
   - Or integrate with external uptime monitoring (e.g., Grafana synthetic monitoring)
