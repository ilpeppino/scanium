...
- Commit changes: `git add -A`, prompt for a commit message, `git commit`, then `git pull --ff-only`.
- Stash changes: `git stash push -u`, then `git pull --ff-only`.
- Discard changes and align with remote: confirm, then `git reset --hard`, `git clean -fd`, and `git pull --ff-only`.
