...
`ScannedItemEntity` (database model) is **missing 4 fields** that exist in `ScannedItem` (domain model). If the database layer is ever activated (currently unused per Issue #002), eBay integration data will be lost.
