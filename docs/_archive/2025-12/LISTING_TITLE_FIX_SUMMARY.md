...
The "Sell on eBay" flow was generating generic listing titles like "Used table" for all items, instead of using the actual scanned item's classification (e.g., "Used Decor / Wall Art", "Used Mug", "Used Bottle").

***REMOVED******REMOVED*** Root Cause
`ListingDraftMapper.fromScannedItem()` was using a simple inline title generator that fell back to `category.displayName` when `labelText` was null/blank. This caused all home goods to show as "Used Home Good" instead of using the specific cloud classification result.
