# Grafana alerting reconcile (NAS)

## Root cause
Grafana alert rule provisioning failed with `UNIQUE constraint failed: alert_rule.guid` and later `alert_rule_version.rule_guid, alert_rule_version.version`. The DB stored empty `guid`/`rule_guid` values, so multiple rules inserted in one provisioning run collided on uniqueness constraints.
...
## Reconcile script
The script deletes specific alert rules by UID using the Grafana provisioning API. It does not print secrets.
...
## DB cleanup (used on NAS)
When API deletes did not clear the conflicts, we performed a DB cleanup that also drops the `UQE_alert_rule_version_rule_guid_version` index.
