...
class CloudClassificationRegressionTest {
    ...
    @Test
    fun testCloudClassification_SuccessfulRequest() = runTest {
        // Arrange - create a test bitmap (no binary asset)
        val testBitmap = generateTestBitmap(
            width = 224,
            height = 224,
    ...
    @Test
    fun testCloudClassification_MultipleRequests() = runTest {
        ...
        val results = colors.mapIndexed { index, color ->
    ...
    @Test
    fun testCloudClassification_ResultContainsExpectedFields() = runTest {
        // Arrange
    ...
    @Test
    fun testCloudClassification_HandlesLargeBitmap() = runTest {
        // Test with a larger bitmap (closer to real camera capture size)
        val largeBitmap = generateTestBitmap(
            width = 640,
            height = 480,
    ...
    @Test
    fun testCloudClassification_DomainPackIdIncluded() = runTest {
        // Verify the domain pack ID is properly included in requests
        val classifier = CloudClassifier(
            context = context,
            domainPackId = "home_resale"
        )
