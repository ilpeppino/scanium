...
class BundleZipExporterTest {
    ...
    fun setUp() {
        ...
        testPhotosDir = File(context.cacheDir, "test_photos").apply {
            mkdirs()
        }
    }
    ...
    @Test
    fun `createZip produces valid ZIP file with correct structure`() = runBlocking {
    ...
    @Test
    fun `createZip includes manifest with correct content`() = runBlocking {
        // Given: test result
    ...
    @Test
    fun `createZip listing_txt contains formatted text`() = runBlocking {
    ...
    @Test
    fun `createZip listing_json contains valid JSON`() = runBlocking {
    ...
    @Test
    fun `createZip handles multiple items correctly`() = runBlocking {
    ...
    @Test
    fun `createZip invokes progress callback`() = runBlocking {
        ...
        var progressCalls = 0
        var stageChanges = 0
        val callback = object : BundleZipExporter.ProgressCallback {
            override fun onProgress(current: Int, total: Int, stage: BundleZipExporter.ExportStage) {
                progressCalls++
            }
    ...
    @Test
    fun `createZip handles items without photos`() = runBlocking {
        // Given: bundle without photos
        val bundle = ExportItemBundle(
            itemId = "no-photos",
    ...
    @Test
    fun `createZip returns correct file size`() = runBlocking {
        // Given: test bundle
