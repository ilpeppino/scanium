...
class DeduplicationPipelineIntegrationTest {
    ...
    @Test
    fun whenSameObjectDetectedWithChangingTrackingIds_thenDeduplicatedCorrectly() =
        runTest(testDispatcher) {
            ...
            repeat(3) { frameNum ->
                val detections =
                    listOf(
                        createDetection(
                            trackingId = "mlkit-track-2",
// NEW tracking ID
                            boundingBox = RectF(105f, 105f, 205f, 205f),
// Slightly moved but similar
                            category = ItemCategory.FASHION,
// Same category
    ...
    @Test
    fun whenMultipleDifferentObjectsScanned_thenAllAddedCorrectly() =
        runTest(testDispatcher) {
            // Test the "no zero items" guarantee - different objects should all be added

            val objectConfigurations =
                listOf(
            ...
            repeat(3) { frameNum ->
                val detections =
                    objectConfigurations.map { (category, box, id) ->
                        createDetection(
                            trackingId = id,
                            boundingBox = box,
                            category = category,
    ...
    @Test
    fun whenTrackingIsImperfect_thenSystemStillAddsItems() =
        runTest(testDispatcher) {
...
// No tracking ID
    ...
    @Test
    fun whenObjectMovesAndReappears_thenStillTrackedCorrectly() =
        runTest(testDispatcher) {
    ...
    @Test
    fun whenSimilarObjectsHaveDifferentCategories_thenBothAdded() =
        runTest(testDispatcher) {
            // Two objects with similar size but different categories

            // Scan first object
            val item1 =
                scanAndConfirmObject(
                    trackingId = "track-1",
                    boundingBox = RectF(100f, 100f, 200f, 200f),
                    category = ItemCategory.FASHION,
                )
    ...
    @Test
    fun whenObjectSizeChangesSignificantly_thenNotConsideredSameObject() =
        runTest(testDispatcher) {
    ...
    @Test
    fun whenConfirmationHappensAcrossMultipleFrames_thenDeduplicationStillWorks() =
        runTest(testDispatcher) {
    ...
    @Test
    fun whenBatchAddingConfirmedItems_thenDeduplicationApplied() =
        runTest(testDispatcher) {
            ...
            repeat(3) { frameNum ->
                val detections =
                    listOf(
                        ...
                        createDetection("track-2", RectF(300f, 100f, 400f, 200f), ItemCategory.FASHION, 0.6f),
// Similar to track-1
    ...
    @Test
    fun whenClearingBetweenScans_thenDeduplicationReset() =
        runTest(testDispatcher) {
            // Scan first session
            repeat(3) { frameNum ->
                val detections =
...
// Similar to track-1
    ...
    @Test
    fun whenManyObjectsScannedRapidly_thenAllProcessedCorrectly() =
        runTest(testDispatcher) {
            ...
            repeat(3) { frameNum ->
                val detections =
                    objectIds.mapIndexed { index, id ->
                        createDetection(
                            trackingId = id,
                            boundingBox =
                                RectF(
                                    100f + index * 150f,
// More spacing to avoid similarity
                                    100f + (index % 2) * 150f,
// Vary Y position too
                                    250f + index * 150f,
                                    250f + (index % 2) * 150f,
    ...
    @Test
    fun whenLowConfidenceDetections_thenFilteredByTracker() =
        runTest(testDispatcher) {
