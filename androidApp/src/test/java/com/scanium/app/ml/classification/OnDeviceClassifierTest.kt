...
class OnDeviceClassifierTest {
    ...
    @Test
    fun `when labeler returns no labels result is unknown`() = runBlocking {
        val classifier = OnDeviceClassifier(
            labelerClient = object : OnDeviceClassifier.ImageLabelerClient {
    ...
    @Test
    fun `when labeler returns label result uses it`() = runBlocking {
        val classifier = OnDeviceClassifier(
            labelerClient = object : OnDeviceClassifier.ImageLabelerClient {
