...
class ListingImagePreparerTest {
    ...
    @Test
    fun `prepareListingImage with valid thumbnail succeeds`() =
        runTest {
            val bitmap = Bitmap.createBitmap(800, 800, Bitmap.Config.ARGB_8888)
            val result =
                preparer.prepareListingImage(
                    itemId = "test-item-1",
                    thumbnail = bitmap,
                )
    ...
    @Test
    fun `prepareListingImage with small thumbnail scales up`() =
        runTest {
    ...
    @Test
    fun `prepareListingImage with no sources fails`() =
        runTest {
            val result =
                preparer.prepareListingImage(
                    itemId = "test-item-3",
    ...
    @Test
    fun `prepareListingImage returns memory cached result when available`() =
        runTest {
    ...
    @Test
    fun `prepareListingImage uses disk cache across instances`() =
        runTest {
            ...
            val cachedResult =
                newPreparer.prepareListingImage(
                    itemId = itemId,
                    fullImageUri = null,
                    thumbnail = null,
                ) as ListingImagePreparer.PrepareResult.Success
