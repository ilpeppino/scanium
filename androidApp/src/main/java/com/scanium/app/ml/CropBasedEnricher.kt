 ...
 */
@Singleton
class CropBasedEnricher @Inject constructor(
    ...
    fun enrichFromCrop(
        ...
        val job = scope.launch(Dispatchers.IO) {
            try {
                ...
                val bitmapToProcess = if (crop.width < MIN_CROP_SIZE_PX || crop.height < MIN_CROP_SIZE_PX) {
                ...
                try {
                    ...
                    Log.i(TAG, "CROP_ENRICH: Local extraction complete - OCR=${localResult.ocrSuccess}, Colors=${localResult.colors.size}")
                ...
                val finalBitmap = if (resultsWeak && bitmapToProcess == crop) {
                ...
                cloudResult.onSuccess { insights ->
                    Log.i(TAG, "CROP_ENRICH: Cloud extraction complete - brand=${insights.visionAttributes.primaryBrand}, itemType=${insights.itemType}")
                    withContext(Dispatchers.Main) {
                        stateManager.applyVisionInsights(
                            aggregatedId = itemId,
    ...
    private fun cropBoundingBox(fullImage: Bitmap, bbox: NormalizedRect): Bitmap {
    ...
    private fun cropWithPadding(fullImage: Bitmap, bbox: NormalizedRect, padding: Float): Bitmap {
