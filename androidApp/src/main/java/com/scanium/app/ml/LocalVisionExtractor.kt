 ...
 */
@Singleton
class LocalVisionExtractor @Inject constructor() {
    companion object {
     ...
     */
    private fun scoreLabelCandidate(text: String, positionIndex: Int): Float {
        var score = 0f

        // Length scoring: prefer medium-length text (5-20 chars is ideal for product names)
        score += when (text.length) {
            in 3..4 -> 0.5f   // Very short, might be fragment
            in 5..10 -> 2.0f  // Ideal range for brand names like "Kleenex"
            in 11..20 -> 1.5f // Good for phrases like "THE ORIGINAL"
            in 21..35 -> 1.0f // Getting long but still ok
            else -> 0.3f      // Too short or too long
        }

        // Case pattern scoring
        val isAllCaps = text.all { !it.isLetter() || it.isUpperCase() }
        val isTitleCase = text.first().isUpperCase() &&
            text.drop(1).any { it.isLowerCase() }
        val hasCapitalWord = text.split(" ").any { word ->
            word.length >= 3 && word.all { !it.isLetter() || it.isUpperCase() }
        }

        when {
            isAllCaps && text.length >= 4 -> score += 1.5f // ALL CAPS like "KLEENEX" or "THE ORIGINAL"
            isTitleCase -> score += 1.0f                    // Title Case like "Kleenex"
        ...
        val recyclingPatterns = listOf(
            "recycl", "cyclabl", "recycle", "recyclable",
            "compost", "biodegr", "sustain",
            "please", "dispose", "trash", "bin",
            "made from", "contains", "ingredients",
        )
        ...
        val productPatterns = listOf(
            "original", "classic", "premium", "deluxe", "ultra",
            "pro", "plus", "max", "extra", "super",
        )
    ...
    private fun getColorName(rgb: Int): String {
        ...
        return when {
            value < 0.2 -> "black"
            value > 0.9 && saturation < 0.1 -> "white"
            saturation < 0.15 -> when {
                value < 0.5 -> "gray"
                else -> "light gray"
            }
            else -> when {
