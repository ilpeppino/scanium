 ...
 */
@Singleton
class PlainTextExporter @Inject constructor(
    ...
    sealed class ExportTextResult {
        data class Success(val text: String, val itemCount: Int) : ExportTextResult()
        data class Error(val message: String) : ExportTextResult()
    }
    ...
    fun generateText(
        ...
        val text = when (format) {
            TextFormat.FULL -> ListingTextFormatter.formatMultiple(result.bundles)
            TextFormat.COMPACT -> ListingTextFormatter.formatCompactList(result.bundles)
            TextFormat.SINGLE -> {
