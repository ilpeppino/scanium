...
data class PricingV3Request(
    ...
    val year: Int? = null,
    val hasAccessories: Boolean? = null,
)

@Serializable
data class PricingV3Response(
    val success: Boolean,
    val pricing: PricingV3PricingDto? = null,
    val cached: Boolean? = null,
    val processingTimeMs: Long? = null,
    val promptVersion: String? = null,
    val error: PricingV3ErrorDto? = null,
)

@Serializable
data class PricingV3PricingDto(
    val status: String,
    val countryCode: String,
    val marketplacesUsed: List<PricingV3MarketplaceDto> = emptyList(),
    val range: PricingV3RangeDto? = null,
...
@Serializable
data class PricingV3MarketplaceDto(
    val id: String,
    val name: String,
...
@Serializable
private data class PricingV3ErrorResponseDto(
    val error: PricingV3ErrorDto? = null,
)
...
class PricingV3Api(
    ...
    suspend fun estimatePrice(
        endpoint: String,
        requestPayload: PricingV3Request,
        apiKey: String,
        ...
        withContext(Dispatchers.IO) {
            val payloadJson = json.encodeToString(PricingV3Request.serializer(), requestPayload)
            ...
            try {
                client.newCall(httpRequest).execute().use { response ->
                    ...
                    if (response.isSuccessful && responseBody != null) {
                        return@use json.decodeFromString(PricingV3Response.serializer(), responseBody)
                    }
                    ...
                    val errorDetails =
                        responseBody
                            ?.let {
                                runCatching {
                                    json.decodeFromString(PricingV3ErrorResponseDto.serializer(), it)
                                }.getOrNull()
                            }?.error
            ...
            } catch (error: PricingV3Exception) {
                throw error
