...
object PhotoPlaybook {
     ...
     */
    private val categoryPlaybooks: Map<ItemCategory, List<PhotoShot>> = mapOf(
     ...
     */
    private val defaultPlaybook = listOf(
        PhotoShot(
            shotType = ShotType.FULL_ITEM,
    ...
    fun getGuidance(
        ...
        val recommendedShots = if (relevantShots.isNotEmpty()) {
            relevantShots
        } else {
            playbook.filter { it.shotType !in takenShotTypes }.sortedBy { it.priority }
        }

        val primaryGuidance = recommendedShots.firstOrNull()?.instruction
     ...
     */
    fun getPlaybookForCategory(category: ItemCategory): List<PhotoShot> {
