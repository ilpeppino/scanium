...
constructor(
    ...
    override suspend fun exists(type: CatalogType, id: String): Boolean = getById(type, id) != null
    ...
    private fun scoreRatio(query: String, label: String): Float =
        if (label.isEmpty()) 0f else query.length.toFloat() / label.length.toFloat()

    private fun result(entry: CatalogEntry, matchType: MatchType, score: Float): CatalogSearchResult =
        CatalogSearchResult(entry = entry, matchScore = score, matchType = matchType)
}
