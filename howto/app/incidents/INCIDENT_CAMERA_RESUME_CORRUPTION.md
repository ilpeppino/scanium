...
2. **Zoomed Fragments**: New captures show cropped/zoomed-in sections of old frames (e.g., "dog
   zoom" instead of full item)
3. **Label Mismatch**: Thumbnail labels don't match image content (e.g., "Adidas" label on wrong
   item thumbnail)
...
- File timestamp would show latest write time (Batch B), but cached thumbnails may point to Batch A
  content
