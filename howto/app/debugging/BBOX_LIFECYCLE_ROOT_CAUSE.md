...
...subsequent coroutine launches in `startPreviewDetection()` may silently fail because the
SupervisorJob's children are cancelled.
...
**Problem:** After `stopPreviewDetection()` sets `isPreviewDetectionActive = false`, the subsequent
`startPreviewDetection()` call (triggered by ON_RESUME) may arrive before:
...
**Problem:** The lifecycle observer in CameraScreen only observes the NavBackStackEntry lifecycle.
When the app goes to background, the NavBackStackEntry may stay RESUMED (device-dependent), but the
camera pipeline should still stop.
...
**Problem:** The CameraXManager instance persists across lifecycle events (correct), but its
...
2. The LaunchedEffect recomposes, but `startPreviewDetection` returns early due to stale
...
3. **ProcessLifecycleOwner Guard**: Handles app-level background/foreground independently of
   navigation
