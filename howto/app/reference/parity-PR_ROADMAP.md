...
**Tracks can run concurrently** as long as dependencies are satisfied. PRs within the same track must be sequential.
...
- Test all Kotlin → Swift conversions: ScannedItem, ImageRef, NormalizedRect, Category, ListingStatus
...
- Attributes: id (String), labelText (String), category (Int32), confidence (Double), timestamp (Date), etc.
