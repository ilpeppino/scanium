...
/env) feeds a typed `CloudClassifierConfig`; `CloudConfigProvider` supplies it per platform.
...
- Android client: `CloudClassifier` uses OkHttp (10s/10s timeouts), multipart JPEG, retries on
  408/429/5xx, strips EXIF.
- Config: `scanium.api.base.url`, `scanium.api.key` from `local.properties`/env → BuildConfig →
  `CloudConfigProvider`.
- Fallback: On-device coarse labels when cloud unavailable/unconfigured; results flagged as
  fallback.
