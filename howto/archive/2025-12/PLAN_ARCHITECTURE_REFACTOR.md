...
**Goal:** Deliver Android-first builds while standing up the shared brain and cloud-first
classification.
...
- Checkpoint commands: `./gradlew assembleDebug`,
...
- Checkpoint commands: `./gradlew :shared:core-models:compileKotlinMetadata`,
  `./gradlew assembleDebug`, `./gradlew test` (where applicable)
...
- Checkpoint commands: `./gradlew test --tests "*Classification*"`, `./gradlew assembleDebug`,
...
- Checkpoint commands: `./gradlew test --tests "*DomainPack*" --tests "*Classification*"`,
  `./gradlew assembleDebug`, `./gradlew lint` (optional)
...
- Contracts/config (Phase 1) can proceed while pipeline hardening starts, as long as adapters shield
  current code.
- Domain pack integration (Phase 3) and backend integration (Phase 4) can run in parallel once
  contracts are stable.
- iOS prep is documentation/contract publishing only; schedule independently of Android feature
  work.
