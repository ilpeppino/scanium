...
**⚠️ CRITICAL:** Do NOT commit this change with actual production credentials if repo is public.
Consider using BuildConfig or environment-based configuration.
...
- **User type:** External
- **Verification:** May require Google verification if requesting sensitive scopes (not required for
  basic profile/email)
- **Scopes required:**
    - `openid`
    - `email`
...
- **Play Console OAuth Requirements:
