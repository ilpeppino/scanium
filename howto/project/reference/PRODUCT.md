...
- Live camera scanning with on-device ML Kit for object, barcode/QR, and document OCR modes (API
  24+).
- Shows detection overlays (boxes + labels) and aggregates stable items with confidence and
  thumbnail snapshots.
- Maintains an item list; users can tap through scan sessions without losing prior detections.
- Export-first flow: select items and share CSV/ZIP exports; marketplace integrations are
...
- Cloud classification pipeline lives in `ml/classification/CloudClassifier.kt` and activates only
  when `SCANIUM_API_BASE_URL`/`SCANIUM_API_KEY` BuildConfig values are provided; otherwise on-device
  labels are used.
