...
- Camera UI/preview/gestures/overlays: `androidApp/src/main/java/com/scanium/app/camera/` (
  CameraScreen, CameraXManager, DetectionOverlay).
- ML analyzers (objects/barcodes/OCR/pricing): `androidApp/src/main/java/com/scanium/app/ml/` (
  ObjectDetectorClient, BarcodeScannerClient, PricingEngine).
- AI Assistant (Claude/OpenAI): `androidApp/src/main/java/com/scanium/app/assistant/` (
  AssistantScreen, AssistantViewModel, AssistantRepository).
- Classification providers: `androidApp/src/main/java/com/scanium/app/classification/` (
  ClassifierProvider, MockClassifier, NoOpClassifier).
- Voice control: `androidApp/src/main/java/com/scanium/app/voice/` (VoiceController,
  VoiceStateMachine, VoiceState).
- Audio feedback: `androidApp/src/main/java/com/scanium/app/audio/` (sound effects, audio feedback).
- FTUE & Onboarding: `androidApp/src/main/java/com/scanium/app/ftue/` (SpotlightTourOverlay,
  WelcomeOverlay, TourViewModel).
- Telemetry export: `androidApp/src/main/java/com/scanium/app/telemetry/` (OpenTelemetry OTLP
  export - logs, traces, metrics).
- Diagnostics & Health: `androidApp/src/main/java/com/scanium/app/diagnostics/` (
  DiagnosticsRepository, system health checks).
- Settings UI: `androidApp/src/main/java/com/scanium/app/settings/` (SettingsScreen,
  DeveloperOptionsScreen).
- Tracking/aggregation logic: `shared/core-tracking/src/commonMain` (ObjectTracker, ItemAggregator)
  via Android wrappers in `core-tracking`.
- Portable models + enums: `shared/core-models/src/commonMain` (ImageRef, NormalizedRect, ScanMode,
  ScannedItem); Android aliases in `core-models`.
- Export models: `shared/core-export/src/commonMain` (ExportItem, ExportPayload, CSV/ZIP mappers).
- Domain pack loading/category mapping: `core-domainpack/src/main/java/com/scanium/app/domain/` (+
  `core-domainpack/src/main/res/raw/home_resale_domain_pack.json`).
- Item state + orchestration: `androidApp/src/main/java/com/scanium/app/items/` (ItemsViewModel,
  ItemsListScreen, EditItemsScreen).
...
- Camera/ML platform adapters:
  `android-platform-adapters/src/main/java/com/scanium/android/platform/adapters/` (Bitmap/ImageRef,
  Rect/NormalizedRect).
- Platform ML/camera shells (namespaces only): `android-ml-mlkit`, `android-camera-camerax` (no
  sources expected).
...
- Domain pack architecture: see `docs/_archive/2025-12/md/architecture/DOMAIN_PACK_ARCHITECTURE.md`
...
Test modules in `androidApp/src/test/java/com/scanium/app/di/FakeModules.kt`.
...
Portability guards: root `checkPortableModules` blocks android imports in `core-models` +
...
- Camera behavior (preview, focus, analyzer thread): `androidApp/camera/CameraXManager.kt`,
  `CameraScreen.kt`, `DetectionOverlay.kt`.
- ML behavior (detectors/pricing): `androidApp/ml/ObjectDetectorClient.kt`,
  `BarcodeScannerClient.kt`, `DocumentTextRecognitionClient.kt`, `PricingEngine.kt`.
- AI Assistant (multimodal chat): `androidApp/assistant/AssistantScreen.kt`,
  `AssistantViewModel.kt`, `AssistantRepository.kt`.
- Classification providers: `androidApp/classification/ClassifierProvider.kt`, `MockClassifier.kt`,
  `NoOpClassifier.kt`.
...
- FTUE/Onboarding: `androidApp/ftue/SpotlightTourOverlay.kt`, `WelcomeOverlay.kt`,
  `TourViewModel.kt`.
- Telemetry export: `androidApp/telemetry/` (OpenTelemetry OTLP - AndroidLogPortOtlp,
  AndroidMetricPortOtlp, AndroidTracePortOtlp).
- Tracking/dedup logic: shared KMP in `shared/core-tracking`; Android entry/usage in `core-tracking`
  and `androidApp/items/ItemsViewModel.kt`.
- Domain pack/category update: `core-domainpack/domain/category/*`,
  `domain/repository/LocalDomainPackRepository.kt`, raw JSON under
  `core-domainpack/src/main/res/raw/`.
...
- Developer Options/System Health: `androidApp/diagnostics/DiagnosticsRepository.kt` (health
  checks), `settings/DeveloperOptionsScreen.kt` (UI).
- Persistence (drafts/DataStore/Room): `androidApp/data/*`, Room entities/DAOs if added; selling
  cache in `selling/data/*`.
...
- **Commit & Push:** When a task is complete and all tests pass, create a commit and push to main.
  ...
  scope).
- **Docker & NAS Operations:** Always use `ssh nas` prefix for any Docker commands or NAS file
  system changes. Examples:
    - `ssh nas "docker compose -p scanium-monitoring restart"`
    - `ssh nas "docker ps -a"`
    - `ssh nas "ls -la /volume1/docker/scanium/"`
    - Never run Docker commands directly on the local machine - the monitoring stack runs on the
      NAS.
- **Mac + NAS Invariant Workflow (Mandatory):**
    - **Repo alignment:** Check `git rev-parse HEAD` on Mac and NAS before any change; stop if they
      differ and align both to the same commit. Never hot-fix runtime state.
    - **Runtime inventory:** Before diagnosing, collect NAS state (`docker ps`, `docker network ls`,
      `docker inspect` for `scanium-backend`, `scanium-alloy`, `scanium-grafana` including networks
      and compose labels).
...
- Build configs: set via `local.properties` or env → `androidApp/build.gradle.kts` (
  ...
  `CLASSIFIER_SAVE_CROPS`). Example defaults in `local.properties.example` (not committed).
- Signing/keystore paths also read from `local.properties` keys (`scanium.keystore.*`).
- Domain packs: JSON under `core-domainpack/src/main/res/raw/`. Never commit secrets or real API
  keys.
