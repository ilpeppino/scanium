...
***REMOVED*** Generate minimal error traffic for monitoring validation
***REMOVED*** Runs for 90 seconds at ~2 rps total
***REMOVED*** Generates 200s, 400s, 401s, 404s for metrics verification

set -euo pipefail

BACKEND_URL="${1:-http://172.23.0.5:8080}"
DURATION_SECONDS=90
INTERVAL=0.5  ***REMOVED*** 2 requests per second total
...
***REMOVED*** Traffic generation loop
while [ "$(date +%s)" -lt "$END_TIME" ]; do
  CYCLE=$((REQUEST_COUNT % 8))

  case $CYCLE in
    0)
      ***REMOVED*** 200 OK - root endpoint
      curl -s -o /dev/null -w "200 OK: %{http_code}\n" "$BACKEND_URL/" &
      ;;
    1)
      ***REMOVED*** 404 Not Found - nonexistent route
      curl -s -o /dev/null -w "404 Not Found: %{http_code}\n" "$BACKEND_URL/this-does-not-exist" &
      ;;
    2)
      ***REMOVED*** 401 Unauthorized - protected endpoint without auth
      curl -s -o /dev/null -w "401 Unauthorized: %{http_code}\n" "$BACKEND_URL/v1/assist/chat" &
      ;;
    3)
      ***REMOVED*** 404 Not Found - another nonexistent route
      curl -s -o /dev/null -w "404 Not Found: %{http_code}\n" "$BACKEND_URL/v1/nonexistent-module" &
      ;;
    4)
      ***REMOVED*** 200 OK - health endpoint
      curl -s -o /dev/null -w "200 OK (health): %{http_code}\n" "$BACKEND_URL/health" &
      ;;
    5)
      ***REMOVED*** 404 Not Found - deep nonexistent path
      curl -s -o /dev/null -w "404 Not Found: %{http_code}\n" "$BACKEND_URL/v1/items/missing/route" &
      ;;
    6)
      ***REMOVED*** 401 Unauthorized - protected classifier endpoint
      curl -s -o /dev/null -w "401 Unauthorized: %{http_code}\n" "$BACKEND_URL/v1/classify" &
      ;;
    7)
      ***REMOVED*** 200 OK - config endpoint
...
***REMOVED*** Wait for background jobs
wait
