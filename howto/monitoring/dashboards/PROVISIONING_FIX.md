...
- The NAS compose file only mounted the Grafana data directory, so provisioning and dashboards from
  the repo were never loaded.
- One dashboard (pipeline-health) used lowercase datasource UIDs that did not match the provisioned
  UIDs.
...
- Reset alerting tables and dropped the `UQE_alert_rule_version_rule_guid_version` index, then added
  triggers to populate `guid`/`rule_guid`.

***REMOVED******REMOVED*** Expected result

- Grafana provisions datasources from
...
- If dashboards are still missing, check Grafana startup logs for provisioning errors and verify the
  mounts.
