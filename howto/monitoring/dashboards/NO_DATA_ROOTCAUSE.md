...
- Backend HTTP metrics and traces were not emitted automatically (no request-level instrumentation
  hook), so even local `/metrics` had no HTTP samples.
- OpenAI dashboards queried `openai_*` metrics that do not exist; the backend exports
  `scanium_assistant_*` instead.
