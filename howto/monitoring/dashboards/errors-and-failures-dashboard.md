...
**Note:** Attempted to add relabel_rules (2026-01-10 22:18:50 UTC), but Alloy v1.0.0's
...
`howto/monitoring/reference/telemetry/MOBILE_TELEMETRY_SCHEMA.md`)
...
Both labels may exist on the same log stream. The dashboard should use `build_type` to match the
schema.
...
All sent to `http://scanium-backend:8080/v1/telemetry/mobile`, all returned 202 Accepted, all
confirmed in backend Docker logs, none ingested into Loki.
