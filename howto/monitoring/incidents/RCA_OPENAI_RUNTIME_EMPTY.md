...
All these metrics are properly defined in `backend/src/infra/observability/metrics.ts` and recorded
in `backend/src/modules/assistant/openai-provider.ts`.
...
```bash
$ curl 'http://localhost:9009/prometheus/api/v1/query?query=scanium_assistant_requests_total'
# Returns 2 series (success + validation error)
# provider="openai", value=5 for success
```
