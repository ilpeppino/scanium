...
ingestion. Mobile dashboards are also empty due to absence of mobile telemetry ingestion.
...
- Backend is deployed via `deploy/nas/compose/docker-compose.nas.backend.yml` (separate compose
  project)
...
- Mobile app OTLP ingestion via Alloy HTTP (port 4318) is configured but no mobile app is actively
  sending telemetry during testing
...
3. **Network Complexity:** Backend and monitoring stacks in separate compose projects with implicit
   network assumptions
