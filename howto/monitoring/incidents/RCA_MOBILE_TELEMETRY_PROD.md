# RCA: Mobile Telemetry Production Implementation
...
- **build_type:** `dev` | `beta` | `prod` (REQUIRED - template variable, note: dashboard calls it "
  build_type" not "flavor")
...
1. **Label-based filtering:** All queries use label matchers, NOT field extraction (except
   session_id)
...
### Test 1: Loki Label Check

```bash
ssh nas "curl -sf 'http://127.0.0.1:3100/loki/api/v1/labels'"
```

**Expected:** If mobile telemetry exists, should see labels: `source`, `platform`, `build_type`,
`app_version`, `event_name`, `session_id`

**Result:** ✅ Labels exist:
`["app_version","build_type","env","event_name","exporter","job","level","platform","source"]`

**Analysis:** Schema is known to Loki (labels exist), but this doesn't mean data exists.

### Test 2: Loki Query for Mobile Events
...
### Test 3: Mimir Metrics Check (if applicable)
...
**Conclusion:** Mobile app has no way to send events, and backend has no endpoint to receive them.
Dashboard is orphaned.
...
1. **6a2d4d2** -
   `feat(telemetry): add prod-ready mobile events endpoint + fix Mobile App Health dashboard + e2e tests`
...
2. **d6d5f17** -
   `fix(telemetry): switch mobile events to Pino OTLP transport + add backend log processor`
...
### Test 1: Backend Event Acceptance ✅
...
1. **Metrics Scraping**: Backend exposes `/metrics` endpoint with `scanium_mobile_events_total`, but
   Alloy scraping needs verification
