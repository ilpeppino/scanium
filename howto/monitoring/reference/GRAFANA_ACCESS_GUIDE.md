...
**Updated**: 2026-01-11
**Grafana URL**: http://192.168.178.45:3000
...
### Main Navigation
- **Home**: http://192.168.178.45:3000/
- **All Dashboards**: http://192.168.178.45:3000/dashboards
- **Scanium Folder**: http://192.168.178.45:3000/dashboards?folderIds=2

### Alternative Folder URLs
Both of these work:
- http://192.168.178.45:3000/dashboards/f/scanium/
- http://192.168.178.45:3000/dashboards/f/scanium/scanium
...
1. **Backend Errors** ⭐ (Fixed today)
   - http://192.168.178.45:3000/d/scanium-backend-errors/scanium-backend-errors
   - Error analysis: 4xx vs 5xx, error routes, logs, traces

2. **Backend Health**
   - http://192.168.178.45:3000/d/scanium-backend-health/scanium-backend-health
   - Health checks, uptime, error rates

3. **Backend API Performance**
   - http://192.168.178.45:3000/d/scanium-backend-api-perf/scanium-backend-api-performance
   - Request latency, throughput, error rates by route

### System Overview

4. **System Overview (RED)**
   - http://192.168.178.45:3000/d/scanium-system-overview/scanium-system-overview-red
   - Rate, Errors, Duration - high-level metrics

5. **Ops Overview**
   - http://192.168.178.45:3000/d/scanium-ops-overview/scanium-ops-overview
   - Operational overview across all services

### Performance & Reliability

6. **Performance & Latency**
   - http://192.168.178.45:3000/d/scanium-scan-performance/scanium-performance-and-latency
   - Latency percentiles, slow requests

7. **Errors & Failures**
   - http://192.168.178.45:3000/d/scanium-errors/scanium-errors-and-failures
   - Cross-service error tracking

8. **Pipeline Health**
   - http://192.168.178.45:3000/d/scanium-pipeline-health/scanium-pipeline-health
   - Data processing pipeline metrics

### Mobile & External Services

9. **Mobile App Health**
   - http://192.168.178.45:3000/d/scanium-mobile-app-health/scanium-mobile-app-health
   - Mobile app telemetry and health

10. **OpenAI Runtime**
    - http://192.168.178.45:3000/d/scanium-openai-runtime/scanium-openai-runtime
    - AI service metrics, latency, costs

### Observability Stack

11. **LGTM Stack Health**
    - http://192.168.178.45:3000/d/scanium-lgtm-health/scanium-lgtm-stack-health
    - Loki, Grafana, Tempo, Mimir monitoring

12. **Logs Explorer**
    - http://192.168.178.45:3000/d/scanium-logs-explorer/scanium-logs-explorer
    - Log search and analysis

13. **Traces Drilldown**
    - http://192.168.178.45:3000/d/scanium-traces-drilldown/scanium-traces-drilldown
...
**Symptom**: Navigating to http://192.168.178.45:3000/dashboards/f/scanium/ doesn't work

**Solutions**:

#### Option 1: Use the Dashboards List (Recommended)
1. Go to http://192.168.178.45:3000/dashboards
...
1. Go to http://192.168.178.45:3000/dashboards?folderIds=2
...
curl http://192.168.178.45:3000/api/health
...
**Expected Behavior**: Grafana is configured with anonymous access enabled. You should NOT see a login prompt.
...
```bash
# List all folders
curl -s "http://192.168.178.45:3000/api/search?type=dash-folder" | jq '.'

# List all dashboards
curl -s "http://192.168.178.45:3000/api/search?type=dash-db" | jq '.'

# List dashboards in Scanium folder
curl -s "http://192.168.178.45:3000/api/search?folderIds=2" | jq '.[] | {title, url}'

# Get folder details
curl -s "http://192.168.178.45:3000/api/folders/scanium" | jq '.'

# Check Grafana health
curl -s "http://192.168.178.45:3000/api/health" | jq '.'
```
